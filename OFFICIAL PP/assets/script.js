...
(() => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    correctInitialHash();
    setTimeout(correctInitialHash, 120);
  });
  window.addEventListener('load', () => {
    document.body.classList.add('loaded');
    correctInitialHash();
    setTimeout(correctInitialHash, 280);
    setTimeout(correctInitialHash, 1000);
  });
  ...
  if (form) {
    form.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        const details = encodeURIComponent(form.querySelector('#details')?.value || '');
