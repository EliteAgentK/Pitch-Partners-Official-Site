...
(() => {
  ...
  function getHeaderOffset() {
    const h = header ? header.getBoundingClientRect().height : 96;
    return Math.ceil(h);
  }
  ...
  function resolveTarget(hash) {
    ...
    if (!id || id === 'top') return document.body;
    const mapped = targetMap[id];
    if (mapped) {
      const found = document.querySelector(mapped);
      if (found) return found;
    }
    const byId = document.getElementById(id);
    if (!byId) return null;
    if (byId.tagName?.toLowerCase() === 'details') {
      byId.open = true;
      return byId.querySelector('summary') || byId;
    }
    if (byId.matches?.('section')) return byId.querySelector('.section-heading, .contact-copy, .consult-form, .legal-intro') || byId;
    return byId;
  }
  ...
  function scrollToTarget(target) {
    ...
    const y = target.getBoundingClientRect().top + window.pageYOffset - getHeaderOffset() - 22;
  ...
  function scrollToHash(hash, updateUrl = true) {
    ...
    // Repeat once after layout settles so the first click is already accurate.
    window.setTimeout(() => scrollToTarget(target), 80);
    window.setTimeout(() => scrollToTarget(target), 260);
  ...
  // Clean root links from WhatsApp/external apps must open at the top.
  function cleanRootToTop() {
  ...
  window.addEventListener('load', () => {
    if (window.location.hash) scrollToHash(window.location.hash, false);
      ...
      cleanRootToTop();
      setTimeout(cleanRootToTop, 120);
      setTimeout(cleanRootToTop, 600);
    }
